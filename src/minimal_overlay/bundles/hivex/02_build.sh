...
cp -r --remove-destination $DEST_DIR/usr/lib/lib* \
  $OVERLAY_ROOTFS/lib
cp -r --remove-destination $DEST_DIR/usr/bin/* \
  $OVERLAY_ROOTFS/bin
echo "copying dependencies"
cp -r --remove-destination /lib/x86_64-linux-gnu/libreadline* \
  $OVERLAY_ROOTFS/lib
cp -r --remove-destination /lib/x86_64-linux-gnu/libtinfo* \
  $OVERLAY_ROOTFS/lib
